...
/// Rewritten
	...
	importOldSettings: function()
	{
		...
		{
			nzbdStatus.setPreference('prompt.sendFile', oldPrefs.getBoolPref('askAboutEnable'));
		}
	...
	onBinSearchLoad: function(doc)
	{
		...
		for (i = 0; i < rowcount; i++)
		{
			...
			results[i].parentNode.parentNode.addEventListener('click', nzbdStatus.rowClick, false);
			url = 'http://binsearch.'+domain+'/?action=nzb&'+postId[1]+'=on';
			if (doc.baseURI.search(/server=(\d)/) > -1)
			{
				url += '&server=' + doc.baseURI.match(/server=(\d)/)[1];
			}
